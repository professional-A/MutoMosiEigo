...
**方針メモ（ユーザー指示 2026-09-10）:** 旧ヘッダーとの「見た目の完全一致」にはこだわらない。実装の簡潔さと天秤にかけ、シンプルな方を採る。テーマの色が通り・崩れず・スマホ幅で破綻しなければ、`.appshell-bar` の背景/余白/ボタン形状などは旧 `.auth-bar` と厳密に一致していなくてよい。ただし id/class 互換（`#notif-badge` `#auth-avatar` `.notif-btn` `#race-view-btn`）は既存JSが依存するので維持する。Task 4/5 の検証も「無スタイル化・機能破壊・レイアウト破綻・コンソールエラーが無いこと」を主眼にし、ピクセル比較は不要。
